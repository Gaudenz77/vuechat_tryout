  ...
  <div class="flex flex-col md:flex-row w-full h-[80vh] my-4">
    ...
    <div class="w-full md:w-3/4 flex flex-col bg-orange-200 p-4 rounded-lg shadow-lg md:ml-4 h-4/5 md:h-full">
          ...
          <div class="flex-1">
            <div class="chat chat-start">
              <div :class="['chat-bubble mt-2 pt-3 pb-4 pe-4', message.userId === props.user.uid ? 'chat-bubble-warning' : 'chat-bubble-info']">
                ...
                {{ message.text }}
                <p><span class="text-xs text-gray-500">{{ message.createdAt ? message.createdAt.toLocaleString() : 'Sending...' }}</span></p>
              </div>
            </div>
          </div>
        </div>
      ...
      <form @submit.prevent="handleSendMessage" class="flex flex-col md:flex-row items-center mt-4">
        ...
        <textarea v-model="messageInput" class="w-full md:w-3/4 rounded-md p-4 text-gray-100 bg-slate-800 resize-none" id="message" @keydown.enter="handleKeyPress" placeholder="Type your message..."></textarea>

        <!-- Emoji Picker Button -->
        <a href="#emoji_modal" class="text-orange-500 text-2xl mt-4 md:mt-0 md:ml-4"><i class="fa-solid fa-face-grin-squint-tears"></i></a>
